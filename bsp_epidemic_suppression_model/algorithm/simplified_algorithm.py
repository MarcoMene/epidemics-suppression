...
def approximated_suppressed_R(R0: float, FTs_infty: float, FTc_infty: float, xi: float, ts: float):
    fraction_of_R0_before_isolation_for_symptoms = gamma_cdf(ts, alpha=r0_alpha, beta=r0_beta)
    symptoms_rescaling = (
            fraction_of_R0_before_isolation_for_symptoms +
            (1 - fraction_of_R0_before_isolation_for_symptoms) * (1 - xi * FTs_infty)
    )
...
def approximated_FTcapp(scapp: float, FTapp_tim1_infty: float, FTnoapp_tim1_infty: float, tildepapp_tim1: float):
    FTcapp_ti_infty = (
                              tildepapp_tim1 * FTapp_tim1_infty +
                              (1 - tildepapp_tim1) * FTnoapp_tim1_infty
                      ) * scapp
...
def simplified_time_evolution(simplified_scenario: SimplifiedScenario, n_iterations: int):
    ...
    Rnoapp = approximated_suppressed_R(R0=simplified_scenario.R0, FTs_infty=FTsnoapp_infty, FTc_infty=FTcnoapp_infty,
                                       xi=simplified_scenario.xi, ts=simplified_scenario.tsnoapp)
    ...
    for i in range(n_iterations):
        ...
        if i == 0:
            ...
            FTcapp_ti_infty = approximated_FTcapp(scapp=simplified_scenario.scapp,
        ...
        R_ti = simplified_scenario.papp * Rapp_ti + (1 - simplified_scenario.papp) * Rnoapp
...
if __name__ == "__main__":
    ...
    simplified_time_evolution(
        simplified_scenario=simplified_scenario,
        n_iterations=5
    )
