from bsp_epidemic_suppression_model.utilities.model import r0, r0asy, r0sy, R0, R0sy, R0asy
...
class TestCornerCases:
    ...
    def test_no_epidemic_control_scenario(self):
        ...
        assert check_equality_with_precision(x=last_step_data.R, y=R0, decimal=precision)
        assert check_equality_with_precision(x=last_step_data.FT_infty, y=0, decimal=precision)
        assert check_equality_with_precision(x=last_step_data.papp, y=last_step_data.tildepapp, decimal=precision)
    ...
    def test_only_simptoms_control(self):
        ...
        last_step_data = step_data_list[-1]
        assert check_equality_with_precision(x=first_step_data.R, y=last_step_data.R, decimal=precision)
